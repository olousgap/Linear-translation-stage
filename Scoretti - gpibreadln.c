...
{   
	...
	char input_buff[131072];																													/* input buffer */
  	char err_buff[160];
  	double term_chr;
  	int max_length=sizeof(input_buff)-1;
	int reason;
	...
	iprintf (instrument, buff);
 	ifread (instrument, input_buff, max_length, &reason, &actual_length);
  	if (reason==I_TERM_MAXCNT) mexErrMsgTxt ("error while reading : input buffer (128Kb) full!!!");
    input_buff[actual_length] = 0;
	p_lhs[0] = mxCreateString (input_buff);
	iclose (instrument);
}
